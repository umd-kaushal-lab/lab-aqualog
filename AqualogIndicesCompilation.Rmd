...
for(i in unique(1:length(eem_list))){
  ...
  eem_list[[i]]$em <- eem_list[[i]]$em[1:475]
  eem_list[[i]]$x <- eem_list[[i]]$x[1:475,]
}

#Take an initial look at the EEMs commented out since it takes a while to load
...
fp.abs.csvs <- file.path(fp.abs, "csvs")
if(!dir.exists(fp.abs.csvs)) {
  dir.create(fp.abs.csvs) 
}
...
for(i in unique((filenames))){
  ...
  write.table(oneabs, file.path(fp.abs,paste("csvs/",name,".csv", sep = "")), sep = ",", row.names = F, col.names = F, quote = F)
}


#Read in absorbance using staRdom
absorbance_path <- fp.abs.csvs
...
problem <- eem_checkdata(eem_list,absorbance,metatable,metacolumns = c("dilution"),error=FALSE)
